...
* バックグラウンドでのページ先読み処理を最適化 (#245)
* 内部の状態管理を改善し、安定性と保守性を向上 (#246)
* 画像・コンテナ処理パイプラインの統合によるメモリ使用率の最適化、およびコミック（ZIP/RAR）のサムネイル生成速度の向上 (#246)

### Fixed

* シリーズ内の本のタイトルが短い場合に「シリーズの並び変え」ダイアログの横幅が不自然に狭くなる不具合を修正 (#243)
* 新しいシリーズやタグを作成した際に、「シリーズを設定」および「タグを設定」ダイアログが意図せず閉じてしまう問題を修正 (#244)
