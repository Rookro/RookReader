...
RookReader は、zip、rar、cbz、cbr、pdf、epub 形式の電子書籍ファイルを閲覧するための、モダンで高速なクロスプラットフォームアプリケーションです。Windows および Linux 環境で利用可能です。
...
* サポートするファイル形式:
  * zip
  * rar
  * cbz (ZIP 形式のコミックアーカイブ)
  * cbr (RAR 形式のコミックアーカイブ)
  * pdf
  * epub (注意: 小説閲覧機能は実験的な機能です。)
* サポートする画像形式 (アーカイブやフォルダー内のページ画像):
  * png / apng
  * jpeg
  * gif
  * webp
  * avif
* 本棚管理機能: 複数の本棚を作成し、独自のアイコンを設定してライブラリを整理できます。
* タグシステム: 本にカスタムタグを付けて、より詳細なカテゴリ分けが可能です。
...
1. リポジトリ内の `PKGBUILD/` から `PKGBUILD` をダウンロードします。
1. ターミナルを開き、そのファイルを置いたディレクトリに移動して、以下のコマンドを実行し、パッケージをビルドおよびインストールします。
...
![reader screenshot](../images/reader_screenshot.png)
...
![bookshelf screenshot](../images/bookshelf_screenshot.png)

* **本棚**
  * 独自の本棚を作成して本を整理できます。
  * ファイルを本棚に追加して管理できます。
  * 名前による検索や、名前・日付での並べ替えが可能です。

* **タグ**
  * タグを使用して、コレクションをさらに細かく分類・フィルタリングできます。

## 開発環境

本プロジェクトは、開発者間の環境差異を最小限に抑えるため、DevContainer を利用した開発フローを推奨しています。

> ネイティブ（コンテナ非利用）環境でのセットアップ手順、前提条件の一覧、Pull Request を出す前に実行すべきチェックについては、[CONTRIBUTING.md](../../CONTRIBUTING.md) を参照してください。
...
貢献を始める前に、[コントリビューションガイド](../../CONTRIBUTING.md) をお読みください。Git Flow のブランチ運用、ローカル環境のセットアップ、コーディング規約、Pull Request を出す前に実行すべきチェックについて説明しています。
