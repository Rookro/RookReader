...
pub fn specta_builder() -> Builder<tauri::Wry> {
    Builder::<tauri::Wry>::new().commands(collect_commands![
        ...
        commands::book_commands::update_reading_direction,
...
#[cfg(test)]
mod error_codes_export {
    use crate::error::ErrorCode;
    use strum::IntoEnumIterator;

    /// Regenerates `src/bindings/errorCodes.ts` from the Rust `ErrorCode` variants.
    ///
    /// Run via `npm run gen:bindings`. CI re-runs this and fails if the working tree
    /// differs, guaranteeing the committed error codes match the backend (SSOT).
    #[test]
    fn export_error_codes() {
        ...
        for variant in ErrorCode::iter() {
            ...
            body.push_str(&format!("  {name}: {code},\n"));
        }
        ...
        std::fs::write("../src/bindings/errorCodes.ts", contents)
            .expect("failed to write errorCodes.ts");

        // Guard against an accidental drop: assert representative entries are present.
        let generated = std::fs::read_to_string("../src/bindings/errorCodes.ts")
            .expect("generated errorCodes.ts should exist");
        for needle in [
            "export const ErrorCode",
            "io: 20001",
            "pathNotFound: 20102",
